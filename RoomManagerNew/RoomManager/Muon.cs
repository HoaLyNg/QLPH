...
namespace RoomManager
{
    public partial class Muon : Form
    {
        ...
        public void connect()
        {
            string cntr = "Server = .\\sqlexpress; Database = QLPH; Integrated Security = true;";
        ...
        {
            connect();
            cm = new SqlCommand("select MaTiet, TenTiet From Tiet Order by TenTiet ASC", cn);
        ...
        public void Timekt()
        {
            ...
            ap.Fill(ds, "Tiet");
            cbbTietKT.DataSource = ds.Tables[0];
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                { throw new Exception("Mã số không được rỗng !"); }
                    ...
                    cmd.Parameters.Add("@ms", txtMa.Text);
                if (txtLyDo.Text.Length == 0)
                { throw new Exception("Lý do không được rỗng !"); }
                else
                    cmd.Parameters.Add("@not", txtLyDo.Text);
                cmd.ExecuteNonQuery();
                ...
                cmd.Parameters.Add("@lap", cbbLaplai.Text);
                cmd.ExecuteNonQuery();
